...
const generateMockDailyRecords = (periodLabel: string): DailyWorkRecord[] => {
  ...
  for (let i = 1; i <= 15; i++) {
    ...
    const actualHours = actualStart && actualEnd ?
      8 + (Math.random() - 0.5) * 2 : 0;
...
export function PayrollDetailModal({
  ...
  const getStatusBadge = (status: DailyWorkRecord["status"]) => {
    switch (status) {
      ...
      case "overtime":
        return { variant: "outline" as const, label: "Overtime", icon: TrendingUp };
      default:
  ...
  const totalScheduledHours = dailyRecords.reduce((sum, record) => sum + record.scheduledHours, 0);
  const totalActualHours = dailyRecords.reduce((sum, record) => sum + record.actualHours, 0);
  const totalAnomalies = dailyRecords.filter(record => record.hasAnomaly).length;
  const attendanceDays = dailyRecords.filter(record => record.actualHours > 0).length;
            ...
            <ScrollArea className="h-[500px] pr-4">
              {payrollData && (
                <div className="space-y-6">
                    ...
                    <Card>
                      <CardContent className="p-4">
                        <div className="text-center">
                          ...
                          <p className="text-lg font-bold">
                            {formatHours(payrollData.currentPeriod.overtimeHours)}
                          </p>
                        </div>
                      </CardContent>
                    </Card>
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-mono">
                                {formatHours(payrollData.currentPeriod.regularHours)}
                              </span>
                            </div>
                            <div className="flex justify-between">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-mono">
                                {formatCurrency(payrollData.currentPeriod.regularPay)}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span>Overtime Pay</span>
                              <span className="font-mono">
                                {formatCurrency(payrollData.currentPeriod.overtimePay)}
                              </span>
                            </div>
                            <Separator />
                            <div className="flex justify-between font-medium">
                              <span>Gross Pay</span>
                              <span className="font-mono">
                                {formatCurrency(payrollData.currentPeriod.grossPay)}
                              </span>
                            </div>
                            <div className="flex justify-between text-red-600">
                              <span>Deductions</span>
                              <span className="font-mono">
                                -{formatCurrency(payrollData.currentPeriod.deductions)}
                              </span>
                            </div>
                            <Separator />
                            <div className="flex justify-between font-bold text-lg">
                              <span>Net Pay</span>
                              <span className="font-mono">
                                {formatCurrency(payrollData.currentPeriod.netPay)}
                              </span>
                            </div>
                          </div>
                        </div>
                  ...
                  {payrollData.previousPeriods && payrollData.previousPeriods.length > 0 && (
                    <Card>
                      <CardHeader>
                      ...
                      <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                          <div className="text-center">
                            <p className="text-muted-foreground">Hours Change</p>
                            <p className="text-lg font-medium">
                              {payrollData.currentPeriod.totalHours > payrollData.previousPeriods[0].totalHours ? (
                                <span className="text-green-600 flex items-center justify-center">
                                  <TrendingUp className="h-4 w-4 mr-1" />
                                  +{(payrollData.currentPeriod.totalHours - payrollData.previousPeriods[0].totalHours).toFixed(1)}h
                                </span>
                              ) : (
                          ...
                          <div className="text-center">
                            <p className="text-muted-foreground">Pay Change</p>
                            <p className="text-lg font-medium">
                              {payrollData.currentPeriod.grossPay > payrollData.previousPeriods[0].grossPay ? (
                                <span className="text-green-600 flex items-center justify-center">
                                  <TrendingUp className="h-4 w-4 mr-1" />
                                  +{formatCurrency(payrollData.currentPeriod.grossPay - payrollData.previousPeriods[0].grossPay)}
                                </span>
                              ) : (
                                <span className="text-red-600 flex items-center justify-center">
                                  <TrendingDown className="h-4 w-4 mr-1" />
                          ...
                          <div className="text-center">
                            ...
                            <p className="text-lg font-medium">
                              {(payrollData.currentPeriod.totalHours / payrollData.currentPeriod.timeEntriesCount).toFixed(1)}h
                            </p>
                          </div>
                        </div>
                      </CardContent>
                    ...
                    return (
                      <Card
                        ...
                        className={`cursor-pointer transition-colors ${
                          record.hasAnomaly ? "border-orange-200 bg-orange-50/30" : ""
                        } ${
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                              <div className="text-center">
                                ...
                                <p className="text-xs text-muted-foreground">{record.dayOfWeek}</p>
                              </div>
                              ...
                              <div className="text-right text-sm">
                                ...
                                {record.scheduledHours > 0 && (
                                  <p className="text-xs text-muted-foreground">
                                    vs {formatHours(record.scheduledHours)} scheduled
                                  </p>
                                )}
                              </div>
                            ...
                            <div className="mt-4 pt-4 border-t">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  ...
                                  <div className="space-y-1 text-sm">
                                    <div className="flex justify-between">
                                      ...
                                      <span>{formatHours(record.actualHours)}</span>
                                    </div>
                                    <div className="flex justify-between">
                                      <span>Break Time:</span>
                                      <span>{formatHours(record.breakTime)}</span>
                                    </div>
                                    {record.scheduledHours > 0 && (
                                      <div className="flex justify-between">
                                        <span>Variance:</span>
                                        <span className={
                                          record.actualHours > record.scheduledHours
                                            ? "text-orange-600"
                                            : record.actualHours < record.scheduledHours - 0.5
                                            ? "text-red-600"
                                            : "text-green-600"
                                        }>
                                          {record.actualHours > record.scheduledHours ? "+" : ""}
                                          {(record.actualHours - record.scheduledHours).toFixed(1)}h
                                        </span>
                                      </div>
                                    )}
                                  </div>
                                ...
                                {record.hasAnomaly && record.anomalyTypes && (
                                  <div>
                                    <h4 className="font-medium mb-2 text-orange-600">Anomalies Detected</h4>
